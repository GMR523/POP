...
class ResNet18_fix(nn.Module):
    def __init__(self, num_classes, pooling=max, model=ResNet18_224x224(num_classes=100), haf_cls_weights=None):
        ...
        with torch.no_grad():
            self.model.fc.bias = nn.Parameter(torch.zeros([self.num_total, ]))
            self.model.fc.bias.requires_grad_(False)
    ...
    def forward(self, x, target="ignored", return_feature=False, return_feature2=False, return_weight=False, return_logit=False):
        # x1 = self.features_2(x)
        logit, feature_list = self.model(x, return_feature_list=True)


        if return_feature:
            return logit, feature_list[-1].view(feature_list[-1].size(0), -1)
        # elif return_feature2:
        #     return logit, feature_10
        elif return_weight:
            return logit, torch.exp(self.temp), self.model.fc.weight, self.haf_cls_weights
        # elif return_logit:
        #     return logit_prev, logit_next
        else:
            return logit
